...
import type { SyncService, RemoteSession } from './sync-service.js';
import { messageOf } from './errors.js';

export function registerCommand(
	ctx: { get: (key: string) => { register: (def: { name: string; description: string; input?: { hint: string }; handler: (inv: { rawInput: string }) => Promise<{ kind: string; text: string }> }) => void } },
	service: SyncService,
): void {

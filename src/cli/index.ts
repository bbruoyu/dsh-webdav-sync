...
async function syncLocalFile(client, localPath, remotePath) {
	...
	try {
		...
		if (!remoteExists) {
			...
			if (remoteDir !== '/') await client.createDirectory(remoteDir, { recursive: true }).catch(() => {});
...
async function pushFile(client, localPath, remotePath) {
	const r = { uploaded: 0, downloaded: 0, unchanged: 0, errors: [] };
	if (!existsSync(localPath)) return r;
	try {
			...
			try {
				...
				for (const sessionId of sessions) {
					...
					for (const fileName of files) {
						...
						tasks.push(pushFile(
							client,
							join(sessionPath, fileName),
							`${cfg.remoteRoot}sessions/${project}/${sessionId}/${fileName}`,
						));
					}
				}
			} catch {}
		...
		for (const proj of projects) {
			...
			for (const sess of sessions) {
				...
				for (const f of files) {
					...
					tasks.push(pullFile(
						client,
						join(DSH_HOME, 'sessions', proj.basename, sess.basename, f.basename),
						`${cfg.remoteRoot}sessions/${proj.basename}/${sess.basename}/${f.basename}`,
					));
				}
			}
		}

...
function mktmp(prefix = 'dsh-wd-test') {
...
function writeSettings(dir, content, version) {
...
class MockWebDAVClient {
    ...
    async putFileContents(path, content, _opts) {
        this.files.set(path, { path, content, mtime: new Date() });
    }
    async createDirectory(_path, _opts) { }
    async getDirectoryContents(path) {
        const entries = Array.from(this.files.entries())
            ...
            return { basename: depth === 1 ? relative : relative.split('/')[0], type: 'file' };
        });
        const seen = new Set();
        return entries.filter(e => { if (seen.has(e.basename))
            return false; seen.add(e.basename); return true; });
    }
}
...
describe('_sync_version helpers', () => {
    ...
    it('should bump version and write back', () => {
        ...
        try {
            ...
            if (m)
                currentVersion = parseInt(m[1], 10);
...
//# sourceMappingURL=sync-logic.spec.js.map
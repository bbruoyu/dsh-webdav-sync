...
function removeEmptyDir(path) {
    try {
        ...
        if (entries.length === 0)
            unlinkSync(path); // will throw if not empty dir; that's fine
    }
    catch { /* ignore */ }
}
/**
 ...
 */
function readSyncVersion(localPath) {
 ...
 */
function bumpSyncVersion(localPath) {
    let content = '';
...
function backupFiles(root, dir = root) {
    const files = [];
    for (const entry of readdirSync(dir, { withFileTypes: true })) {
        ...
        if (entry.isDirectory())
            files.push(...backupFiles(root, fullPath));
 ...
 */
export function createProfileBackup(profile, dir, opts) {
    ...
    const files = backupFiles(root).sort().map((rel) => {
        const content = readFileSync(join(root, rel), 'utf8');
        return rel === 'package.json'
            ? { path: rel, json: JSON.parse(content) }
...
export function validatedBackup(value) {
    ...
    if (backup.files.length > MAX_BACKUP_FILES)
        throw new Error('invalid backup contents');
    const files = [];
    const paths = new Set();
    for (const raw of backup.files) {
        ...
        paths.add(path);
        if (SKIP_NAMES.has(path.split('/')[0]))
        ...
        if (path === 'package.json') {
            ...
            if (!Array.isArray(file.lines) || !file.lines.every(l => typeof l === 'string'))
 ...
 */
export function restoreProfileBackup(value, dir) {
...
export class SyncService {
    ...
    get _cfg() {
        ...
        return {
            ...
            conflictStrategy: String(base.conflictStrategy ?? 'newer-wins'),
    ...
    async _ensureClient() {
        ...
        try {
            ...
            console.error(`[${TAG}] Connect failed:`, err.message ?? err);
            this._client = null;
            return false;
        }
    }
    ...
    async sync() {
        ...
        const result = { uploaded: 0, downloaded: 0, unchanged: 0, conflicts: 0, errors: [] };
        try {
     ...
     */
    async uploadSession(cwd, sessionId) {
        if (this._syncing)
            return { uploaded: 0, downloaded: 0, unchanged: 0, conflicts: 0, errors: ['already-running'] };
        this._syncing = true;
        const start = Date.now();
     ...
     * @param sessionId - session ID
     */
    async downloadSession(cwd, sessionId) {
        if (this._syncing)
            return { uploaded: 0, downloaded: 0, unchanged: 0, conflicts: 0, errors: ['already-running'] };
        this._syncing = true;
        const start = Date.now();
        ...
        try {
            ...
            // List remote files
            const remoteFiles = await client.getDirectoryContents(remoteSessionDir).catch(() => []);
            const logFiles = remoteFiles.filter((f) => f.type === 'file' && f.basename.endsWith('.jsonl.zstd'));
        ...
        try {
            ...
            for (const proj of projects) {
                if (proj.type !== 'directory')
                    continue;
                const sessionsList = await client.getDirectoryContents(`${cfg.remoteRoot}sessions/${proj.basename}`).catch(() => []);
            ...
            console.error(`[${TAG}] listRemoteSessions failed:`, err.message);
            return [];
        }
    }
    ...
    async uploadBackup(backupPath) {
        ...
        try {
            ...
            if (parentDir !== '/' && parentDir !== '.') {
                ...
                try {
                    await client.createDirectory(collectionUrl, { recursive: true });
                }
                catch { /* exists */ }
            }
     ...
     */
    async downloadAndRestore(backupPath) {
        if (this._syncing)
            return { uploaded: 0, downloaded: 0, unchanged: 0, conflicts: 0, errors: ['already-running'] };
        this._syncing = true;
        const start = Date.now();
    ...
    async testConnection() {
        try {
            ...
            return { ok: false, remoteUrl: this._cfg.remoteUrl, error: err.message ?? String(err) };
        }
    }
    // ── internal helpers ──────────────────────────────────────────────────────
    _merge(target, src) {
     ...
     */
    async _syncLocalFile(client, localPath, remotePath) {
        const r = { uploaded: 0, downloaded: 0, unchanged: 0, conflicts: 0, errors: [] };
        try {
            ...
            try {
                ...
                try {
                    ...
                    r.errors.push(`${localPath}: ${uploadErr.message}`);
                }
            }
        }
        catch (err) {
    ...
    async _readRemoteSyncVersion(client, remotePath) {
        try {
    ...
    _bumpAndUpload(client, localPath, remotePath) {
        bumpSyncVersion(localPath);
        return this._uploadFile(client, localPath, remotePath);
    }
    async _pullLocalFile(client, localPath, remotePath) {
        const r = { uploaded: 0, downloaded: 0, unchanged: 0, conflicts: 0, errors: [] };
        try {
    ...
    async _syncDir(client, localDir, remoteDir) {
        const r = { uploaded: 0, downloaded: 0, unchanged: 0, conflicts: 0, errors: [] };
        ...
        try {
            ...
            r.errors.push(`Dir ${localDir}: ${err.message}`);
        }
        return r;
    }
    async _pullDir(client, localDir, remoteDir) {
        const r = { uploaded: 0, downloaded: 0, unchanged: 0, conflicts: 0, errors: [] };
        try {
    ...
    async _syncSessions(client, cfg) {
        const r = { uploaded: 0, downloaded: 0, unchanged: 0, conflicts: 0, errors: [] };
        ...
        try {
            ...
            r.errors.push(`Sessions: ${err.message}`);
        }
        return r;
    }
    async _pullSessions(client, cfg) {
        const r = { uploaded: 0, downloaded: 0, unchanged: 0, conflicts: 0, errors: [] };
        try {
            const projects = await client.getDirectoryContents(`${cfg.remoteRoot}sessions`).catch(() => []);
            for (const proj of projects) {
            ...
            r.errors.push(`Sessions pull: ${err.message}`);
        }
        return r;
    }
    async _uploadFile(client, localPath, remotePath) {
